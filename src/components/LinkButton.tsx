...
const LinkButton = ({ label, type, nav, className, haveIcon = true }: ILinkButton) => {
	return (
		<Link
		...
		>
			{label === 'Əlavə Et! +' ? (
				<button
					className={`w-[180px] flex rounded-full justify-center items-center py-2.5 gap-4 text-qss-secondary border-2`}
				>
					{label}
				</button>
			) : (
				<button
					className={`w-[180px] flex rounded-full justify-center items-center py-3.5 gap-4 ${type ===
					'outline'
